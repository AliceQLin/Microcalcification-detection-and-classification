...
class focal_loss(nn.Module):
    # 1223 5867 70 11
    def __init__(self, alpha=[0.25, 1, 0.2, 5, 10], gamma=2, num_classes=5, size_average=True):
        """
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # print(class_logits.size(), labels.size())
    # classification_loss = F.cross_entropy(class_logits, labels)
    fl = focal_loss()
    classification_loss = fl(class_logits, labels) * 2
...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self, class_logits, box_regression, proposals, image_shapes):
        ...
        for boxes, scores, image_shape in zip(pred_boxes, pred_scores, image_shapes):
            ...
            # modify to nms for all classes
            # print(labels.size())
            keep = box_ops.batched_nms(boxes, scores, torch.zeros(labels.size()[0]), self.nms_thresh)
